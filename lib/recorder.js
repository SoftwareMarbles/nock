...
function generateRequestAndResponse(req, bodyChunks, options, res, dataChunks) {
  ...
  if (requestBody) {
    ret.push(', ');
    try {
      //  Try to parse the body to check if its JSON and if it is output it as stringified JSON.
      ret.push(JSON.stringify(JSON.parse(requestBody)));
    } catch(err) {
      //  Otherwise output it as stringified value (string, number, even object).
...
function record(rec_options) {
  ...
  //  To preserve backward compatibility (starting recording wasn't throwing if nock was already active) we:
  //    1. restore any requests that may have been overridden by other parts of nock (e.g. intercept)
  //    2. override ClientRequest again as recording also relies on it.
  //  NOTE: This is hacky as hell but it keeps the backward compatibility *and* allows correct
  //    behavior in the face of other modules also overriding HTTP/HTTPS requests.
  common.restoreOverriddenRequests();
  intercept.restoreOverriddenClientRequest();
  intercept.overrideClientRequest();
