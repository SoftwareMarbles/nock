...
function record(rec_options) {
  ...
  var thisRecordingId = recordingId;
  ...
  common.overrideRequests(function(proto, overriddenRequest, options, callback) {
    ...
    var req = overriddenRequest(options, function(res) {
      ...
      res.once('end', function() {
        ...
        debug(thisRecordingId, proto, 'intercepted request ended', JSON.stringify(rec_options));
        ...
        debug(thisRecordingId, proto, 'recording', typeof(out), 'value to outputs');
        outputs.push(out);
...
function restore() {
  debug(recordingId, 'restoring all the overridden http/https properties');
