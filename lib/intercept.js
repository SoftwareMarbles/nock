...
function interceptorsFor(options) {
  ...
  //  HACK: First try to use filteringScope if any of the interceptors has it defined.
...
function activate() {
  // ----- Extending http|https.ClientRequest
  ...
  https.ClientRequest = OverridenClientRequest;
  ...
  common.overrideRequests(function(proto, overriddenRequest, options, callback) {
    ...
    if (isOn() && interceptors.length) {
      ...
      if (! matches && allowUnmocked) {
        return overriddenRequest(arguments);
      }
      ...
      if (isOff() || isEnabledForNetConnect(options)) {
        //  We pass undefined as the overridenRequest is bound to its module.
        return overriddenRequest.call(undefined, options, callback);
      } else {
        throw new NetConnectNotAllowedError(options.host);
      }
    }
