...
function helpersLoadNockDefs(filename) {
...
function load(path) {
  ...
  var nockDefs  = helpersLoadNockDefs(path);
  var nocks     = []
  ...
  nockDefs.forEach(function(n) {
    ...
    var nscope     = n.scope
      , npath      = n.path
      , method     = n.method.toLowerCase() || "get"
      , status     = n.reply || 200
      , response   = n.response ? JSON.parse(n.response) : ''
      , headers    = n.headers    || {}
    ...
    var nock;
    if(body==="*") {
      nock = startScope(nscope).filteringRequestBody(function() {
        return "*";
      })[method](nscope, "*").reply(status, response, headers);
    } else {
      nock = startScope(nscope);
      if(reqheaders !== {}) {
